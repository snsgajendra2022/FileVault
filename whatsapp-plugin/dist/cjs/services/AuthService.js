...
class AuthService {
    constructor(config, eventEmitter, gateway) {
        ...
        logger_1.logger.info('AuthService initialized (gateway available: ' + gateway.isAvailable() + ')');
    }
    async initialize() {
        try {
            ...
            logger_1.logger.error('Failed to initialize AuthService:', error);
            throw error;
        }
    }
    /**
    ...
    async startQRLogin(force = false) {
        logger_1.logger.info('Starting QR login process (force: ' + force + ')');
        if (this.gateway.isAvailable()) {
    ...
    async waitForQRScan() {
        ...
        if (this.gateway.isAvailable()) {
            ...
            if (connected) {
                ...
                logger_1.logger.info('QR code scanned successfully, phone: ' + this.phoneNumber);
            }
            return connected;
        }
        // Fallback: mock scan
        return new Promise((resolve) => {
            setTimeout(async () => {
                ...
                await this.saveSession();
                logger_1.logger.info('QR code scanned successfully (mock)');
                resolve(true);
            }, 5000);
        });
    }
     ...
     */
    async logout() {
        logger_1.logger.info('Logging out from WhatsApp');
        if (this.gateway.isAvailable()) {
            try {
                await this.gateway.logoutWhatsApp();
            }
            catch (error) {
                logger_1.logger.warn('Gateway logout failed:', error.message);
            }
        }
        this.isAuthenticatedFlag = false;
        this.phoneNumber = null;
    ...
    async saveSession() {
        try {
            ...
            if (this.config.sessionStorage) {
                ...
                logger_1.logger.info(`Would save session to ${this.config.sessionPath}`);
            }
    ...
    async loadSession() {
        try {
            ...
            if (this.config.sessionStorage) {
                ...
                logger_1.logger.info(`Would load session from ${this.config.sessionPath}`);
            }
            if (sessionData?.isAuthenticated) {
                ...
                if (sessionAge < maxAge) {
                    ...
                    logger_1.logger.info('Session expired, clearing');
                    await this.clearSession();
                }
            }
            return false;
    ...
    async getMockQRCode() {
      ...
      <text x="100" y="140" text-anchor="middle" font-size="10" fill="#999">Dev mode — not scannable</text>
        ...
        const btoaFn = typeof btoa !== 'undefined' ? btoa : (s) => Buffer.from(s).toString('base64');
        return {
