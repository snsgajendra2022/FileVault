...
class FileService {
    ...
    async sendFile(to, file, caption) {
        ...
        logger_1.logger.info(`Sending file to ${to}: ${fileName}`);
        if (this.gateway.isAvailable()) {
            // Gateway requires a file path on disk
            // If we have a Buffer, we'd need to write it first
            const filePath = file?.path || null;
            if (!filePath) {
                logger_1.logger.warn('Gateway media send requires a file path on disk. Upload file first.');
                return {
                    success: false,
        ...
        logger_1.logger.info(`File sent (simulated) to ${to} with ID: ${messageId}`);
    ...
    async sendMediaFromPath(to, mediaPath, caption) {
        logger_1.logger.info(`Sending media from path to ${to}: ${mediaPath}`);
        if (this.gateway.isAvailable()) {
    ...
    async handleInboundMedia(mediaData) {
        logger_1.logger.info('Received inbound media', mediaData);
