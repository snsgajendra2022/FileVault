 ...
 */
export class Config {
  private static defaults: WhatsAppPluginConfig = {
    // Connection settings
   ...
   */
  private static loadFromEnvironment(): WhatsAppPluginConfig {
    const envConfig: Partial<WhatsAppPluginConfig> = {};
    ...
    if (process.env.WHATSAPP_PLUGIN_LOG_LEVEL) {
      const level = process.env.WHATSAPP_PLUGIN_LOG_LEVEL.toLowerCase() as any;
    ...
    return envConfig as WhatsAppPluginConfig;
  }
