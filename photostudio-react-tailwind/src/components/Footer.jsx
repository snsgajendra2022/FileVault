...
export default function Footer() {
  return (
    <footer className="w-full border-t border-slate-800 bg-slate-900 py-10 text-white md:py-12">
      <div className="container-page">
        <div className="flex flex-col justify-between gap-8 md:flex-row md:items-start">
          ...
          <nav className="flex flex-wrap gap-x-5 gap-y-2 text-sm font-medium text-slate-400">
            {links.map((link) => (
              <a key={link.label} href={link.href} className="transition hover:text-blue-500 text-white">
                {link.label}
              </a>
            ))}
          </nav>
        ...
        <div className="mt-8 border-t border-slate-800 pt-5 text-center text-xs text-white md:text-left">
