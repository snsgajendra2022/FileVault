...
/** Guest capabilities for the shared event link (sent with public-share payload when supported). */
export type ShareGuestPermissions = {
...
export type PublicShareModalLabels = {
  ...
  allowImageUploadLabel: string;
  allowViewEventImagesLabel: string;
};
...
interface PublicShareModalProps {
  ...
  onGuestPermissionsChange: (next: ShareGuestPermissions) => void;
...
const EventPublicShareModal: React.FC<PublicShareModalProps> = ({
  ...
  const defaultLabels: PublicShareModalLabels = {
    ...
    guestAccessTitle: t('memoriesPlatform.shareGuestAccessTitle'),
    allowImageUploadLabel: t('memoriesPlatform.sharePermImageUpload'),
    allowViewEventImagesLabel: t('memoriesPlatform.sharePermViewImages'),
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
      <div className="bg-white rounded-2xl shadow-xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-4 space-y-4">
          ...
          <div>
            <p className="block text-sm font-medium text-gray-700 mb-2">{L.guestAccessTitle}</p>
            <div className="flex flex-col gap-2 sm:flex-row sm:gap-6">
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  type="checkbox"
                  checked={guestPermissions.allowImageUpload}
                  onChange={(e) =>
                    onGuestPermissionsChange({ ...guestPermissions, allowImageUpload: e.target.checked })
                  }
                  className="rounded border-gray-300"
                />
                <span className="text-sm">{L.allowImageUploadLabel}</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  type="checkbox"
                  checked={guestPermissions.allowViewEventImages}
                  onChange={(e) =>
                    onGuestPermissionsChange({
                      ...guestPermissions,
                      allowViewEventImages: e.target.checked,
                    })
                  }
                  className="rounded border-gray-300"
                />
                <span className="text-sm">{L.allowViewEventImagesLabel}</span>
              </label>
            </div>
          </div>
            </>
...
export default EventPublicShareModal;
