...
const PhotoThemeCategoryPage: React.FC = () => {
  ...
  return (
    <div className="space-y-8 w-full">
      ...
      {/* All themes created by this user (from /api/covers) */}
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold text-gray-900">Your saved themes</h2>
          {isLoadingUserThemes && (
            <span className="text-xs text-gray-500">Loading themes...</span>
          )}
        </div>
        {userThemesError && (
        ...
        {userCoverThemes.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {userCoverThemes.map((theme) => (
              <div
                ...
                className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 flex flex-col gap-3 cursor-pointer hover:shadow-md hover:border-indigo-300 transition"
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm font-semibold text-gray-900">
                      {theme.frontCover?.headline || 'Untitled cover'}
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3 text-xs">
                  <div className="rounded-xl bg-slate-900 text-white p-3 space-y-1">
                    ...
                    <p className="font-semibold line-clamp-2">
                      {theme.frontCover?.headline || '-'}
                    </p>
                    <p className="text-[11px] text-slate-200 line-clamp-2">
                      {theme.frontCover?.subheadline || '-'}
                    </p>
                  </div>
                  <div className="rounded-xl bg-slate-100 text-slate-900 p-3 space-y-1">
          ...
          <p className="text-xs text-gray-500">
            You haven&apos;t saved any themes yet. Once you create and save covers, they will appear
            here.
          </p>
        ) : null}
      </div>
